...
public class OptimisticReservationRepositoryImpl implements ReservationRepository {
    private final ReservationJPARepository optimisticReservationJPARepository;

    @Override
    public void save(Reservation reservation){
        optimisticReservationJPARepository.save(reservation);
    }

    @Override
    public Optional<Reservation> findReservationByUserIdAndSeatIdAndPaymentId(String userId, Long seatId, Long paymentId){
        return optimisticReservationJPARepository.findReservationByUserIdAndSeatIdAndPaymentId(userId, seatId, paymentId);
    }

    @Override
    public Optional<List<Reservation>> findReservationsByUserIdAndPaymentIdWithLock(String userId, Long paymentId){
        return optimisticReservationJPARepository.findReservationsByUserIdAndPaymentIdForUpdateWithOptimisticLock(userId, paymentId);
    }
    ...
    public Optional<Reservation> findReservationBySeatIdAndStatusWithLock(Long seatId, ReservationStatusType reservationStatusType){
        return optimisticReservationJPARepository.findReservationBySeatIdAndReservationStatusForUpdateWithOptimisticLock(seatId, reservationStatusType);
    }
}
